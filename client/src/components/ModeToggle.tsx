...
export default function ModeToggle({ mode, onChange }: Props) {
  return (
    <div className="flex mx-4 mb-3 bg-[#1A1A1A] rounded-xl p-1 gap-1 border border-white/10">
      <button
        onClick={() => onChange('fuel')}
        className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-sm font-bold transition-all tracking-wide ${
          mode === 'fuel'
      ...
      <button
        ...
        className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-sm font-bold transition-all tracking-wide ${
          mode === 'ev'
            ? 'bg-[#00E5FF] text-[#111111] shadow-lg shadow-[#00E5FF]/30'
            : 'text-white/40 hover:text-white/70'
        }`}
        style={{ fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: '0.06em' }}
      >

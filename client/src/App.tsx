...
export default function App() {
  ...
  // EV stations effect — fetch when location changes or mode switches to EV
  useEffect(() => {
  ...
  // Re-score fuel stations when fuel type changes
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50">
      {/* Header */}
      <header className="flex-shrink-0 bg-[#0f2744]">
        <div className="px-4 pt-3 pb-1 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white font-bold tracking-tight">TankVergelijker</span>
          </div>
          {isLoading && (
            <div className="flex items-center gap-1.5 text-slate-400 text-xs">
              <span className="block w-3 h-3 border-2 border-slate-600 border-t-orange-400 rounded-full animate-spin" />
      ...
      {state.status === 'error' && state.errorMessage && (
        <div className="flex-shrink-0 mx-3 mt-2 px-3 py-2.5 bg-red-50 border border-red-200 rounded-xl text-sm text-red-600 flex items-center gap-2">
